...
export function VideoPublisher({ userId, providerApproved, onPublished }: { userId: string; providerApproved: boolean; onPublished: () => Promise<void> }) {
  ...
  async function publish(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (selectedType === "video") {
        ...
        setPhase("publishing");
        ({ error } = await supabase.rpc("create_video_post", {
  ...
  return <article className="panel video-publisher-panel" id="video-publisher">
    ...
    <form className="dashboard-form video-publisher-form" onSubmit={(event) => void publish(event)}>
      ...
      {contentType === "video" ? <label className="wide video-drop">Vidéo MP4, WebM ou MOV · 90 s maximum · 100 Mo<input name="video" type="file" accept="video/mp4,video/webm,video/quicktime" required onChange={(event) => { previewUrls.forEach((url) => URL.revokeObjectURL(url)); setPreviewUrls(event.target.files?.[0] ? [URL.createObjectURL(event.target.files[0])] : []); }} /></label> : <label className="wide video-drop">{contentType === "before_after" ? "Deux images : avant puis après" : "Images JPEG, PNG ou WebP · 10 maximum"}<input name="media" type="file" accept="image/jpeg,image/png,image/webp" multiple={contentType !== "promotion" && contentType !== "availability"} required onChange={(event) => { previewUrls.forEach((url) => URL.revokeObjectURL(url)); setPreviewUrls(Array.from(event.target.files ?? []).map((item) => URL.createObjectURL(item))); }} /></label>}
      {previewUrls.length > 0 && <div className="wide publisher-preview">{contentType === "video" ? <video src={previewUrls[0]} controls muted playsInline aria-label="Aperçu de la vidéo" /> : previewUrls.map((url, index) => <Image key={url} src={url} alt={contentType === "before_after" ? index === 0 ? "Avant" : "Après" : `Aperçu ${index + 1}`} width={320} height={400} unoptimized />)}<span>Aperçu avant publication</span></div>}
