...
type CreatorStat = { post_id: string; views: number; average_watch_ms: number; completion_rate: number; likes: number; comments: number; saves: number; booking_clicks: number; bookings: number; gross_amount: number };
...
export function SocialDashboard({ role, userId }: { role: "client" | "provider"; userId: string }) {
  ...
  if (role === "provider") return <article className="panel" id="creator-statistics"><div className="panel-heading"><div><h2>Statistiques créateur</h2><p>Interactions et réservations réellement attribuées aux vidéos.</p></div><button onClick={() => void load()}>Actualiser</button></div>{stats.length === 0 ? <div className="compact-empty">Publiez une vidéo pour commencer à mesurer son impact.</div> : <div className="creator-stat-grid">{stats.map((item) => <section key={item.post_id}><small>Vidéo {item.post_id.slice(0, 8)}</small><strong>{item.views.toLocaleString("fr-FR")} vues</strong><span>{(item.average_watch_ms/1000).toFixed(1)} s vues en moyenne · {Number(item.completion_rate).toFixed(1)} % complétées</span><span>{item.likes} likes · {item.comments} commentaires · {item.saves} sauvegardes</span><span>{item.booking_clicks} clics Réserver · {item.bookings} réservations</span><b>{item.gross_amount.toLocaleString("fr-FR")} FCFA générés</b></section>)}</div>}</article>;
