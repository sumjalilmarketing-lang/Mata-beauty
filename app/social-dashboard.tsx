...
type CollectionPost = { collection_id: string; post_id: string };
...
export function SocialDashboard({ role, userId }: { role: "client" | "provider"; userId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    const [collectionResult, savedResult, collectionPostsResult] = await Promise.all([
  ...
  async function assignCollection(postId: string, collectionId: string) {
    const supabase = getSupabaseBrowserClient();
    if (!supabase || !collectionId) return;
    const { error } = await supabase.from("inspiration_collection_posts").upsert({ collection_id: collectionId, post_id: postId });
    setFeedback(error ? "Impossible d’ajouter cette inspiration à la collection." : "Inspiration ajoutée à la collection.");
    if (!error) await load();
  }
  ...
  return <article className="panel" id="inspirations"><div className="panel-heading"><div><h2>Mes inspirations</h2><p>Vidéos enregistrées, professionnels favoris et collections personnelles.</p></div></div><form className="collection-form" onSubmit={(event) => void createCollection(event)}><input name="name" required maxLength={80} aria-label="Nom de la collection" placeholder="Nouvelle collection" /><button>Créer</button></form>{feedback && <p className="dashboard-feedback" role="status">{feedback}</p>}<div className="collection-pills">{collections.map((collection) => <span key={collection.id}>{collection.name} · {collectionPosts.filter((item) => item.collection_id === collection.id).length}</span>)}</div>{saved.length === 0 ? <div className="compact-empty">Enregistrez une vidéo depuis le feed pour la retrouver ici.</div> : <div className="saved-post-grid">{saved.map((item) => <article key={item.post_id}>{item.posts?.thumbnail_url ? <Image src={item.posts.thumbnail_url} alt="" width={320} height={240} unoptimized /> : <span>▶</span>}<p>{item.posts?.caption ?? "Inspiration beauté"}</p><select aria-label={`Collection pour ${item.posts?.caption ?? "cette inspiration"}`} defaultValue="" onChange={(event) => void assignCollection(item.post_id, event.target.value)}><option value="">Ajouter à une collection</option>{collections.map((collection) => <option key={collection.id} value={collection.id}>{collection.name}</option>)}</select><a href={`/feed?post=${item.post_id}`}>Ouvrir et réserver</a></article>)}</div>}</article>;
}
