...
export async function GET(request: Request) {
  ...
  if (!wallet) return Response.json({ ok: true, balance: { currency: "XOF", pending: 0, available: 0, earned: 0, paidOut: 0 }, entries: [] });
  ...
  return Response.json({ ok: true, balance, entries });
}
