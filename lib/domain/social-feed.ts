...
export const socialFeedFilters = ["Pour toi", "Abonnements", "Près de moi", "Tendances", "Make-up", "Barbier", "Tresses", "Perruques", "Ongles", "Coiffure", "Soins"] as const;
...
export function matchesSocialFeedFilter(filter: SocialFeedFilter, post: { city: string; serviceTitle: string | null; hashtags: string[] }, nearbyCity = "Dakar") {
  ...
  if (filter === "Abonnements") return true;
  ...
  const aliases: Record<Exclude<SocialFeedFilter, "Pour toi" | "Abonnements" | "Tendances" | "Près de moi">, string[]> = {
